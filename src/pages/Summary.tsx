...
const Summary: React.FC = () => {
  ...
  const exportSummaryPDF = () => {
    try {
      ...
      const procedureData = procedureSummary.map(summary => [summary.procedure, summary.count.toString(), Object.entries(summary.supervisionBreakdown).map(([type, count]) => `${type}: ${count}`).join(', ')]);
      autoTable(doc, {
      ...
      const hospitalTableData = hospitalData.map(item => [item.name, item.value.toString()]);
      autoTable(doc, {
        head: [['Hospital', 'Count']],
  ...
  return <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Statistical overview of your procedures</p>
        </div>
      ...
      {loading ? <div className="grid gap-6 grid-cols-1 md:grid-cols-2">
        ...
        </div> : records.length === 0 ? <Card className="glass-card">
              ...
              <CardContent>
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={supervisionData} margin={{
                  top: 20,
                  right: 30,
                  left: 20,
                  bottom: 5
                      ...
                      <Tooltip />
                      <Legend />
                      {['Direct', 'Indirect', 'Independent', 'Teaching'].map((type, index) => <Bar key={type} dataKey={type} stackId="a" fill={COLORS[index % COLORS.length]} />)}
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
            ...
            <Card className="glass-card">
              ...
              <CardContent>
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={hospitalData} cx="50%" cy="50%" labelLine={true} label={({
                    name,
                    percent
                  }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={80} fill="#8884d8" dataKey="value">
                        {hospitalData.map((_, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                      </Pie>
            ...
            <CardContent>
              <div className="h-[350px]">
                ...
                bottom: 5
                    ...
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="count" stroke="#0088FE" activeDot={{
                  r: 8
                }} />
                  </LineChart>
                </ResponsiveContainer>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {procedureSummary.map((summary, index) => <tr key={index} className="border-b">

...
const Dashboard: React.FC = () => {
  ...
  return <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-900">Welcome to IVF Logbook Pro</h2>
        ...
        <Card className="glass-card">
          ...
          <CardContent>
            <div className="text-3xl font-bold">
              {loading ? <div className="h-4 w-16 animate-pulse bg-gray-200 rounded"></div> : stats.totalRecords}
            </div>
          </CardContent>
        </Card>
        
        {topProcedures.slice(0, 3).map((procedure, index) => <Card key={index} className="glass-card">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">
            ...
            <CardContent>
              <div className="text-3xl font-bold">
                {loading ? <div className="h-4 w-16 animate-pulse bg-gray-200 rounded"></div> : procedure.count}
              </div>
            </CardContent>
          </Card>)}
      </div>
        ...
        <Card className="glass-card">
          ...
          <CardContent>
            {loading ? <div className="space-y-4">
              ...
              </div> : stats.recentRecords.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                <p>No records added yet</p>
              </div> : <div className="relative overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead className="text-xs uppercase bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 bg-slate-900">Date</th>
                      <th className="px-6 py-3 bg-slate-900">MRN</th>
                      <th className="px-6 py-3 bg-slate-900">Procedure</th>
                      <th className="px-6 py-3 bg-slate-900">Hospital</th>
                      <th className="px-6 py-3 bg-slate-900">Supervision</th>
                    </tr>
                  </thead>
                  <tbody>
                    {stats.recentRecords.map(record => <tr key={record.id} className="bg-white border-b hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap bg-slate-700">{record.date}</td>
                        <td className="px-6 py-4 bg-slate-700">{record.mrn}</td>
                        <td className="px-6 py-4 bg-slate-700">{record.procedure}</td>
                        <td className="px-6 py-4 bg-slate-700">{record.hospital}</td>
                        <td className="px-6 py-4 bg-slate-700">{record.supervision}</td>
                      </tr>)}
                  </tbody>
                </table>
              </div>}
